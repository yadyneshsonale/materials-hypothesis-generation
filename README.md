...
4. The sufficiency assessor decides whether to accept the evidence, reason with a stated
  caveat, search a narrower question, record a conservative assumption, or leave the question
  unresolved.
...
| Context assembler | Attach source passages, tables, conditions, and citation status |
| Sufficiency assessor | Route evidence to accept, caveat, decompose, or unresolved |
| Gap resolver | Create narrower child tasks or record conservative assumptions |
...
4. **Solve each subtask adaptively:** retrieve and rerank evidence, then make a structured
  decision: `sufficient`, `reason_with_caveat`, `decompose_further`, or `unresolved`.
  Inference generated by the model must be labeled separately from retrieved
  literature claims.
...
expansion, context assembly, sufficiency routing, one-level recursive gap decomposition,
autonomous assumption recording, conflict adjudication, cited synthesis, criticism, and a complete
JSON execution trace. The
