...
flowchart TD
  ...
  U[Hypothesis request] --> H[Normalize goal and assume missing constraints conservatively]
   ...
   F --> K
   G --> K
  K --> L[Assemble passages, tables, and citation context]
  L --> CA[Resolve citations to extracted papers]
  E --> CR[Select relevant roles from cited papers]
  CA --> CR
  CR --> M{Evidence sufficient?}
  ...
  M -->|Missing constraint| Q[Record conservative assumption]
...
2. The planner-orchestrator turns that request into connected literature questions and schedules
  them according to their dependencies.
3. The evidence researcher retrieves claims, attaches passages and tables, resolves citations to
  locally extracted papers, promotes relevant cited-paper roles, and assesses sufficiency.
4. Missing constraints receive conservative recorded assumptions; evidence gaps trigger narrower
  tasks or remain explicitly unresolved without pausing for user input.
...
| --- | --- |
| Planner-orchestrator | Decompose the goal, schedule dependent tasks, apply autonomous defaults, and trace execution |
| Evidence researcher | Retrieve and rerank claims, assemble context, resolve citations, promote cited roles, and assess sufficiency |
...
| Critic | Score plausibility, novelty, feasibility, testability, and evidence coverage |
...
2. **Normalize the request:** convert the desired hypothesis into an explicit target
  material or class, desired property, operating conditions, constraints, novelty scope,
  and acceptable validation methods. Missing constraints receive conservative defaults that
  are recorded as assumptions in the trace.
...
4. **Solve each subtask adaptively:** retrieve and rerank direct evidence, resolve citations
  attached to selected passages and tables, and promote relevant roles from locally extracted
  cited papers into first-class evidence. Then make a structured decision: `sufficient`,
  `reason_with_caveat`, `decompose_further`, or `unresolved`. Inference generated by the model
  must be labeled separately from retrieved literature claims.
...
expansion, context assembly, citation resolution, cited-role promotion, sufficiency routing,
one-level recursive gap decomposition, autonomous assumption recording, conflict adjudication,
cited synthesis, criticism, and a complete JSON execution trace. The
...
.venv/bin/python source_context.py \
  ...
  --context-dir source_context --metadata results/new_candidates.json
```

When a passage or table cites another paper in the extracted corpus, source-context
generation resolves the reference by arXiv ID, DOI, or conservative title matching and
attaches the cited paper's relevant extracted roles and evidence spans. Agents therefore
receive grounded claims from the cited paper rather than a paper title alone. Unmatched
references remain routing metadata and are not treated as substantive evidence.
